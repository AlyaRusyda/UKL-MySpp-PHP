...
<body>
    ...
    <?php while ($data_siswa = mysqli_fetch_assoc($result)) { ?>
        <p>NISN : <?= $data_siswa['nisn'] ?><br>
        Nama : <?= $data_siswa['nama'] ?><br>
        Kelas : <?= $data_siswa['nama_kelas'] ?><br>
        Telp : <?= $data_siswa['no_tlp'] ?><br>
        Angkatan : <?= $data_siswa['angkatan'] ?><br>
        Alamat : <?= $data_siswa['alamat'] ?></p>
        ID Petugas : <?= $_SESSION['id_petugas'] ?>
    <?php
    }
    ?>
    ...
    <table class="table table-hover my-0">
        ...
        <tbody>
            ...
            inner join spp on spp.id_spp = pembayaran.id_spp and 
            pembayaran.nisn = '".$_GET['nisn']."' ORDER BY bulan_spp ASC");
            $no = 0;
